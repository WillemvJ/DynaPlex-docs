...
- **interpreter round-trip** — a function was compiled, ingested, and run on the
  DynaPlex interpreter (the core pipeline works even without the JIT).
...
.. note::
   ...
   **source** (via ``inspect.getsource``), which only exists for functions
   defined in a real ``.py`` file. That is exactly why the self-test ships as a
   module — use ``python -m dynaplex.selftest`` rather than trying to reproduce
   it inline.
...
``RESULT: OK`` (JIT skipped)  Core works; this wheel has no JIT (unusual for
                              the published wheels — expected only for a
                              JIT-free build).
``RESULT: FAILED``            Something is wrong; the failing check and its
                              error are printed above the summary.
============================  ===============================================

If the self-test fails, please open an issue and include the full output
(the environment block and the per-check lines):
https://github.com/WillemvJ/DynaPlex-docs/issues
